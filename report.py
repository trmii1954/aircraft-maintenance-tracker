...
def pdf_report(self, report_type):
    ...
    c = SimpleDocTemplate(
        f"{self.folder}/reports/{self.registration} {report_type} Report.pdf", pagesize=letter
    )
    ...
    # Add a title
    title = Paragraph(
        f"{self.registration}", custom_styles["Title"]
    )
    ...
    content.append(
        Paragraph(
            f"Last Flight on {self.last_flight_date}"
        )
    )
    ...
    content.append(
        Paragraph(
                "Major Systems and Their Subsystems", custom_styles["Subtitle"]
        )
    )
    ...
    if report_type in ( "all", "configuration"):
            ...
            content.append(Paragraph("<font size=18 color='green'>Configuration.<br/>Sorted by Last Maintenance Date</font>", custom_styles["Subsubtitle"]))
    ...
    if report_type in ( "all", "maintenance"):
            ...
            content.append(PageBreak())
            content.append(Paragraph("<font size= 18 color='green'>Maintenance Status.<br/>Sorted by Maintenance Due Date</font>", custom_styles["Subsubtitle"]))
            content.append(Paragraph("<font size=10 color='lightgrey'>ITIS = Interval Time in Service Interval</font>"))
            content.append(Spacer(1, 12))

            for s in self.major_systems:
                together = [Paragraph(f"<font size=14 color='blue'>{s.title()} System</font>")]
                together.append(Spacer(1, 12))
                together = together + report_maintenance(self.major_systems[s],self.flight_hours)
                together.append(Spacer(1, 12))
    ...
    if report_type in ( "all", "log"):
            for s in sorted(self.major_systems):
                content.append(PageBreak())
                content.append(Paragraph(f"Major System: <font size=18 color='green'>{s.title()}</font>", custom_styles["Subsubtitle"]))
                content.append(Spacer(1, 6))
                content += report_log(self.major_systems[s])
                content.append(Paragraph(f"<font size=14 color='brown'>{s.title()} Subsystems Log<br/>(sorted by subsystem)</font>", custom_styles["Subsubtitle"]))
                for ss in dict(sorted(self.major_systems[s].subsystems.items())).values():
                    together = [Paragraph(f"{ss.subsystem_name}", custom_styles["Subsubtitle"])]
                    together = together + report_log(ss)
                    content = content + [KeepTogether(together)]

    # Build the PDF
    disclaim = "WARNING: See Logbooks for Definitive Information."
    c.build(content, onFirstPage=make_header_footer(report_type,disclaim), onLaterPages=make_header_footer(report_type, disclaim))
...
def report_system(system):
    ...
    r.append(Paragraph(f"Subsystems: {", ".join(sorted(list(system.subsystems.keys())))}"))
...
def report_subsystem(system):
    ...
    for ss in system.subsystems.keys():
        t.append([ss,system.subsystems[ss].make,\
                        system.subsystems[ss].model, \
                        system.subsystems[ss].serial,\
                        system.subsystems[ss].maintenance_date
                        ])

    sorted_t = sorted(t, key=lambda maintenance_date: maintenance_date[-1], reverse=True)
    table = Table(sorted_t)
    style = TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.Color(red=0.0, green=0.4, blue=0.8)),  # Header row background
...
def maintenance_table(system, flight_hours):
    ...
    sorted_table = sorted(table, key=lambda x: make_sort_key(x[6])) #x[6] is due_in_at_s
    hdr = ["Subsystem","Last Maint","@Flt Hours","Interval","Interval TIS","Remaining","Due At/On"]
...
def make_header_footer(report_type, disclaim):
    def header_footer(canvas, doc):
        ...
        canvas.drawString(doc.pagesize[0] - 250, doc.pagesize[1] - 40, f"{disclaim.title()}")
        canvas.setFillColor(black)

        # Add a footer
        canvas.setFont("Helvetica", 10)
        canvas.drawString(0.5*inch, 0.5*inch, f"Page {doc.page}")
        canvas.drawRightString(8.0*inch, 0.5*inch, f"{datetime.now(timezone.utc)} UTC")
        canvas.restoreState()
